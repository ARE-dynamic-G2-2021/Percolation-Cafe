...
def initialisation(): #generer/regenerer notre environnement; boutton Reset relié a initialisation  (1)
    ...
    lbl_percolation.place_forget() #enleve l'afficheur perco car on reset
...
def propagation(): #permet de lancer/faire progresser l'animation  (6)
    ...
    if nb_eau == 0: #si arret de propagation, arreter l'animation de l'eau
        running = False
        return 
...
def ajouter_eau(event): #ajouter de l'eau  (5)
    ...
    i, j = event.y // taille_cellule, event.x // taille_cellule
    if world[0][j] == 1: #permet d'ajouter de l'eau QUE SI on a un pore
        world[0][j]= 2
        dessiner_une_cellule(world, i, j)
        ...
        if world[1][j1] == 3: 
            for j2 in range(nb_cellules):
                if world[nb_cellules - 1][j2] == 3:  #si on trouve du cafe en bas, il y a eu percolation
                    lbl_percolation.place(x = 804, y = 210) #on place notre afficheur de perco
...
lbl_percolation = Label(window, text = "Percolation", font = 9, bg = "lawn green", width = 9, height = 2) #afficheur percolation
...
slider = Scale(window, orient = "vertical", command = change_slider, from_ = 100, to = 0, length = 220, width = 35 , font = 2) #slider
